...
▀█████████▄           ██       ████████
██▀ │FlassTas ﹏ @FlassTas
│تم تطوير وبرمجة السورس من قبل FlassTas .
│@FlassTas
#-------------------------------------------------------------------
]]
...
FlassTas = Token:match("(%d+)")
redis:set(FlassTas..":VERSION",1)
redis:set(FlassTas..":SUDO_ID:",SUDO_USER)
redis:set(FlassTas..":DataCenter:",'German')
redis:set(FlassTas..":UserNameBot:",BOT_User)
redis:set(FlassTas..":NameBot:",BOT_NAME)
redis:hset(FlassTas..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3FlassTas_INSTALL","Yes")
info = {}
...
FlassTas = Token:match("(%d+)")
our_id = tonumber(FlassTas)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(FlassTas..":UserNameBot:")
SUDO_ID = tonumber(redis:get(FlassTas..":SUDO_ID:"))
SUDO_USER = redis:hgetall(FlassTas..'username:'..SUDO_ID).username
version = redis:get(FlassTas..":VERSION")
DataCenter = redis:get(FlassTas..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..FlassTas..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(FlassTas..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(FlassTas..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(FlassTas..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(FlassTas..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(FlassTas..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(FlassTas..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(FlassTas..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(FlassTas..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(FlassTas..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(FlassTas..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(FlassTas..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(FlassTas..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(FlassTas..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(FlassTas..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(FlassTas..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس FlassTas ⇐ @FlassTas10
...
if msg.GroupActive and redis:get(FlassTas..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(FlassTas..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(FlassTas..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(FlassTas..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.FlassTas then 
if msg.text and ScriptFile.iFlassTas then
for k, FlassTas in pairs(ScriptFile.FlassTas) do
local SearchText = msg.text:match(FlassTas)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iFlassTas(msg,{msg.text:match(FlassTas)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",FlassTas.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dFlassTas then
if ScriptFile.dFlassTas(msg) == false then
return false
end
...
if Plug.FlassTas then 
if msg.text and Plug.iFlassTas then
for k, FlassTas in pairs(Plug.FlassTas) do
local SearchText = msg.text:match(FlassTas)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(FlassTas..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(FlassTas..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iFlassTas(msg,{msg.text:match(FlassTas)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",FlassTas.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.dFlassTas then
Plug.dFlassTas(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source FlassTas")
end 

end
else
print("The File Script.lua Not Runing in The Source FlassTas")
...
function tdcli_update_callback(data)
	...
	redis:srem(FlassTas..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/Alamy300/FlassTas/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/Alamy300/FlassTas/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/Alamy300/FlassTas/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/Alamy300/FlassTas/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/Alamy300/FlassTas/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/Alamy300/FlassTas/main/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(FlassTas..":SUDO_ID:")
redis:hset(FlassTas..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(FlassTas..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(FlassTas..'mute_tgservice'..msg.chat_id_) then
	Del_msg(msg.chat_id_,msg.id_)
	end
	elseif msg.content_.ID == "MessageChatAddMembers" then
	if redis:get(FlassTas..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(FlassTas..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(FlassTas..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(FlassTas..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(FlassTas..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(FlassTas..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(FlassTas..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(FlassTas..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(FlassTas..'group:add'..msg.chat_id_) then
	redis:set(FlassTas..'group:name'..msg.chat_id_,msg.content_.title_)
	end
	end 
	
	elseif data.ID == "UpdateNewChat" then  
	if redis:get(FlassTas..'group:add'..data.chat_.id_) then
	redis:set(FlassTas..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(FlassTas..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(FlassTas..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(FlassTas..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	FlassTas..'group:name'..k,IDS.Title,
	FlassTas..'num_msg_FlassTas'..k,5,
	FlassTas..'group:add'..k,true,
	FlassTas..'lock_link'..k,true,
	FlassTas..'lock_spam'..k,true,
	FlassTas..'lock_webpage'..k,true,
	FlassTas..'lock_markdown'..k,true,
	FlassTas..'lock_flood'..k,true,
	FlassTas..'lock_bots'..k,true,
	FlassTas..'mute_forward'..k,true,
	FlassTas..'mute_contact'..k,true,
	FlassTas..'mute_document'..k,true,
	FlassTas..'mute_inline'..k,true,
	FlassTas..'lock_username'..k,true,
	FlassTas..'replay'..k,true
	)
	redis:sadd(FlassTas..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(FlassTas..'username:'..ID,'username',user)
	redis:sadd(FlassTas..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(FlassTas..'username:'..ID,'username',user)
	redis:sadd(FlassTas..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(FlassTas..'username:'..ID,'username',user)
	redis:sadd(FlassTas..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(FlassTas..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(FlassTas..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(FlassTas..'username:'..data.user_.id_).username
	...
	redis:hset(FlassTas..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
	...
	local groups = redis:smembers(FlassTas..'group:ids')
	...
	local pv = redis:smembers(FlassTas..'users')
	local NumPvDel = 0
	for i = 1, #pv do
