...
message = '- 𖠲قائمه الاعضاء المميزين :\n'   
for k,v in pairs(list) do
local info = redis:hgetall(sonic..'username:'..v)
if info and info.username and info.username:match("@[%a%d_]+") then
message = message ..k.. '~⪼ '..(info.username or '')..' « ❪`' ..v.. '`❫ \n'
...
if #list==0 then return "- 𖠲لايوجد اعضاء مكتومين " end
message = '- 𖠲قائمه الاعضاء المكتومين :\n'
for k,v in pairs(list) do
local info = redis:hgetall(sonic..'username:'..v)
if info and info.username and info.username:match("@[%a%d_]+") then
message = message ..k.. '~⪼ '..(info.username or '')..' « ❪`' ..v.. '`❫ \n'
...
if #list==0 then return "- 𖠲لايوجد أعضاء محظورين " end
message = '- 𖠲قائمه الاعضاء المحظورين :\n'
for k,v in pairs(list) do
local info = redis:hgetall(sonic..'username:'..v)
if info and info.username and info.username:match("@[%a%d_]+") then
message = message ..k.. '~⪼ '..(info.username or '')..' « ❪`' ..v.. '`❫ \n'
...
local list = redis:smembers(sonic..':Filter_Word:'..msg.chat_id_)
if #list == 0 then return "- 𖠲قائمه الكلمات الممنوعه فارغه" end
filterlist = '- 𖠲قائمه الكلمات الممنوعه :\n'    
for k,v in pairs(list) do
...
return  "- 𖠲الكلمه *{"..word.."}* هي بالتأكيد من قائمه المنع " 
else
redis:sadd(sonic..':Filter_Word:'..msg.chat_id_,word) 
return  "- 𖠲الكلمه *{"..word.."}* تمت اضافتها الى قائمه المنع  "
end
end
...
return  "- 𖠲الكلمه *{"..word.."}* تم السماح بها  " 
else
return  "- 𖠲الكلمه *{"..word.."}* هي بالتأكيد مسموح بها " 
end
end
...
sendMsg(msg.chat_id_,1,'- 𖠲عذرا لديك الكثير من المجموعات\n- 𖠲سوف ارسل لك ملف فيها قائمه مجموعات المفعله انتظر لحظه ...')
...
if redis:get(sonic..'group:add'..msg.chat_id_) then  return '•𖠲المجموعه بالتأكيد تم تفعيلها' end
...
return sendMsg(msg.chat_id_,msg.id_,'-𖠲لآ يمـگنني تفعيل آلبوت في آلمـجمـوعه يجب آن يگون آگثر مـن *【'..GroupUsers..'】* عضـو ')
else 
GetChatMember(msg.chat_id_,our_id,function(arg,data)
if data.status_.ID == "ChatMemberStatusMember" then
return sendMsg(msg.chat_id_,msg.id_,'- 𖠲عذرا البوت ليس ادمن  في المجموعه •\n-𖠲يرجى ترقيته ادمن لتتمكن من تفعيل البوت  ')
else
...
USERNAME_T = '•𖠲الـمعرف ❪ @['..datai.username_..'] ❫\n'
else 
USERNAME_T = ''
end
...
..'-𖠲الاسـم ❪ ['..FlterName(datai.first_name_..' '..(datai.last_name_ or ""),23)..'](tg://user?id='..msg.sender_user_id_..') ❫\n'
...
if not redis:get(sonic..'group:add'..msg.chat_id_) then return '- 𖠲 المجموعه بالتأكيد تم تعطيلها' end  
rem_data_group(msg.chat_id_)
return '•𖠲The group has been disabled .'
end

function modrem(msg)
if not msg.SudoUser then return '•𖠲أنـت لـسـت الـمـطـور'end
if not redis:get(sonic..'group:add'..msg.chat_id_) then return '•𖠲المجموعه بالتأكيد تم تعطيلها' end  
rem_data_group(msg.chat_id_)
...
if cmd =="tqeed" then
if UserID == our_id then   
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك تقييد البوت\n•") 
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك تقييد المطور الاساسي\n•")
elseif  UserID == 1186900224 then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك تقييد مطور السورس\n•")
elseif redis:sismember(sonic..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك تقييد المطور\n•") 
elseif redis:sismember(sonic..':KARA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك تقييد المنشئ\n•") 
elseif redis:sismember(sonic..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك تقييد المنشئ\n•") 
elseif redis:sismember(sonic..'owners:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك تقييد المدير\n•") 
elseif redis:sismember(sonic..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك تقييد الادمن\n•") 
end
...
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنني تقييد المشرف\n") 
end
end)
end
...
if cmd =="fktqeed" then
if UserID == our_id then  
return sendMsg(ChatID,MsgID,"- 𖠲البوت ليس مقييد  \n𖠲") 
end
...
if UserID == our_id then 
return sendMsg(ChatID,MsgID,"- 𖠲عذرا لا يمكنني رفع نفسي \n𖠲") 
elseif data.type_.ID  == "UserTypeBot" then 
return sendMsg(ChatID,MsgID,"- 𖠲عذرا لا يمكن رفع بوت في البوت \n𖠲") 
elseif data.type_.ID == "chatTypeChannel" then 
return sendMsg(ChatID,MsgID,"- 𖠲عذرا لا يمكن رفع قناة في البوت \n𖠲") 
end
...
if cmd == "ban" then
if UserID == our_id then   
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك حظر البوت\n𖠲") 
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك حظر المطور الاساسي\n𖠲") 
elseif  UserID == 1186900224 then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك حظر المطور الاساسي\n𖠲") 
elseif  UserID == 1312825214 then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك حظر المطور الاساسي\n𖠲") 
elseif  UserID == 1153357069 then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك حظر مطور السورس\n𖠲") 
elseif redis:sismember(sonic..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك حظر المطور\n𖠲") 
elseif redis:sismember(sonic..':KARA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك حظر المنشئ\n•") 
elseif redis:sismember(sonic..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك حظر المنشئ\n𖠲") 
elseif redis:sismember(sonic..'owners:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك حظر المدير\n𖠲") 
elseif redis:sismember(sonic..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك حظر الادمن\n𖠲") 
end
kick_user(UserID,ChatID,function(arg,data)
if data.ID == "Error" and data.code_ == 400 then
return sendMsg(ChatID,MsgID,'- 𖠲لا يمكنني حظر العضو .\n- 𖠲لانه مشرف في المجموعه \n 𖠲')    
elseif data.ID == "Error" and data.code_ == 3 then
return sendMsg(ChatID,MsgID,'- 𖠲لا يمكنني حظر العضو .\n- 𖠲ليس لدي صلاحيه الحظر او لست مشرف\n 𖠲')    
else
redis:hset(sonic..'username:'..UserID, 'username', Resolv)
...
if cmd == "silent" then
if UserID == our_id then   
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك كتم البوت\n𖠲") 
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك كتم المطور الاساسي\n𖠲") 
elseif  UserID == 1186900224 then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك كتم المطور الاساسي\n𖠲") 
elseif  UserID == 1312825214 then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك كتم المطور الاساسي\n𖠲") 
elseif  UserID == 1153357069 then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك كتم مطور السورس\n𖠲") 
elseif redis:sismember(sonic..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك كتم المطور\n𖠲") 
elseif redis:sismember(sonic..':KARA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك كتم المنشئ\n•") 
elseif redis:sismember(sonic..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك كتم المنشئ\n𖠲") 
elseif redis:sismember(sonic..'owners:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك كتم المدير\n𖠲") 
elseif redis:sismember(sonic..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك كتم الادمن\n𖠲") 
end
...
if cmd == "kick" then
if UserID == our_id then   
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك طرد البوت\n𖠲") 
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك طرد المطور الاساسي\n𖠲") 
elseif  UserID == 1186900224 then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك طرد المطور الاساسي\n𖠲") 
elseif  UserID == 1312825214 then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك طرد المطور الاساسي\n𖠲") 
elseif  UserID == 1153357069 then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك طرد مطور السورس\n𖠲") 
elseif redis:sismember(sonic..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك طرد المطور\n𖠲") 
elseif redis:sismember(sonic..':KARA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك طرد المنشئ\n•") 
elseif redis:sismember(sonic..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك طرد المنشئ\n𖠲") 
elseif redis:sismember(sonic..'owners:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك طرد المدير\n𖠲") 
elseif redis:sismember(sonic..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"- 𖠲لا يمكنك طرد الادمن\n𖠲") 
end
kick_user(UserID, ChatID,function(arg,data)
if data.ID == "Error" and data.code_ == 400 then
...
if data.status_.ID == "ChatMemberStatusMember" then 
Restrict(ChatID,UserID,1)  
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲تم تقييد آلعضـو بنجآح \n ') 
else
return sendMsg(ChatID,MsgID,'- 𖠲لا يمكنني تقييد العضو .\n- 𖠲لانه مشرف في المجموعه \n 𖠲')    
end
end)
else
return sendMsg(ChatID,MsgID,'- 𖠲لا يمكنني تقييد العضو .\n- 𖠲لانني لست مشرف في المجموعه \n 𖠲')    
end
end)
...
if data.status_.ID == "ChatMemberStatusEditor" then 
Restrict(ChatID,UserID,2)
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲تم فك تقييد آلعضـو بنجآح \n ') 
else
return sendMsg(ChatID,MsgID,'- 𖠲لا يمكنني قك تقييد العضو .\n- 𖠲لانني لست مشرف في المجموعه \n 𖠲')    
end
end)
end,nil)
end
...
if redis:sismember(sonic..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲انه بالتأكيد منشىء في البوت \n ') 
end
redis:hset(sonic..'username:'..UserID, 'username', UserName)
redis:sadd(sonic..':MONSHA_BOT:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲تمت ترقيته ليصبح منشىء في البوت \n ')
end

if cmd == "remmnsha" then
if not redis:sismember(sonic..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲انه بالتأكيد ليس منشىء \n ') 
end
redis:srem(sonic..':MONSHA_BOT:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲تمت تنزيله من المنشىء \n ') 
end
...
if redis:sismember(sonic..'whitelist:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'🙍🏻‍♂*- *العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n-𖠲انه بالتأكيد من عضو مميز \n ') 
end
redis:hset(sonic..'username:'..UserID, 'username', UserName)
redis:sadd(sonic..'whitelist:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲تمت ترقيته ليصبح ضمن عضو مميز \n ')
end

if cmd == "remwhitelist" then
if not redis:sismember(sonic..'whitelist:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲انه بالتأكيد ليس من عضو مميز \n ') 
end
redis:srem(sonic..'whitelist:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲تمت تنزيله من عضو مميز \n ')
end
...
redis:hset(sonic..'username:'..UserID, 'username', UserName)
redis:sadd(sonic..'admins:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲تمت ترقيته ليصبح ادمن \n ')
end

if cmd == "remowner" then
if not redis:sismember(sonic..'owners:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲انه بالتأكيد ليس مدير \n ') 
end
redis:srem(sonic..'owners:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲تم تنزيله من الاداره \n ')
end

if cmd == "demote" then
if not redis:sismember(sonic..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲انه بالتأكيد ليس ادمن \n ') 
end
redis:srem(sonic..'admins:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲تم تنزيله من الادمنيه \n ')
end
...
if UserID == our_id then 
return sendMsg(ChatID,MsgID,"- 𖠲عذرا لا يمكنني حظر نفسي \n𖠲") 
elseif data.type_.ID == "ChannelChatInfo" then 
return sendMsg(ChatID,MsgID,"- 𖠲عذرا لا يمكن حظر قناة \n𖠲") 
end
...
if cmd == "unban" then
if not redis:sismember(sonic..'banned:'..ChatID,UserID) then 
sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲تم بالتأكيد الغاء حظره \n ') 
else
sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲تم الغاء حظره \n ')
end
redis:srem(sonic..'banned:'..ChatID,UserID)
...
if cmd == "unbanall" then
if not GeneralBanned(UserID) then 
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲تم بالتأكيد الغاء حظره العام \n ') 
end
redis:srem(sonic..'gban_users',UserID)
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲تم الغاء حظره العام \n ')
end
...
redis:hset(sonic..'username:'..UserID, 'username', UserName)
redis:sadd(sonic..':SUDO_BOT:',UserID)
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲تم ترقيته ليصبح مطور \n✓')
end

if cmd == "dn_sudo" then
if not redis:sismember(sonic..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n𖠲نه بالتأكيد ليس مطور \n✓')
end
redis:srem(sonic..':SUDO_BOT:',UserID)
return sendMsg(ChatID,MsgID,'- 𖠲العضو « ❪ '..UserName..' ❫\n- 𖠲الايدي « ❪ `'..UserID..'` ❫\n- 𖠲تم تنزيله من المطورين \n✓') 
end

else
return sendMsg(ChatID,MsgID,"- 𖠲 لآ يوجد عضـو بهذآ آلمـعرف \n𖠲")
end 
...
if cmd == "dn_sudo" then
if not redis:sismember(sonic..':SUDO_BOT:',UserID) then return sendMsg(ChatID,MsgID,'-𖠲العضو « '..USERNAME..' \n-𖠲 الايدي « (`'..UserID..'`)\n-𖠲انه بالتأكيد ليس مطور \n✓') end
redis:srem(sonic..':SUDO_BOT:',UserID) 
return SendMention(ChatID,UserID,MsgID,'-العضو « ❪ '..USERNAME..' ❫\n-الايدي « ❪ '..UserID..' ❫\n-تم تنزيله من المطورين \n ',17,USERCAR) 
end
...
if redis:sismember(sonic..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"-𖠲لا يمكنك كتم المدراء او الادمنيه\n𖠲") 
end
...
.."\n1-╿التعديل «« "..(redis:get(sonic..'lock_edit'..msg.chat_id_) or 'false')
.."\n2-│الروابط «« "..(redis:get(sonic..'lock_link'..msg.chat_id_) or 'false')
.."\n3-│التاك «« "..(redis:get(sonic..'lock_tag'..msg.chat_id_) or 'false')
.."\n4-│المعرفات «« "..(redis:get(sonic..'lock_username'..msg.chat_id_) or 'false')
.."\n5-│التكرار «« "..(redis:get(sonic..'lock_flood'..msg.chat_id_) or 'false')
.."\n6-│الكلايش «« "..(redis:get(sonic..'lock_spam'..msg.chat_id_) or 'false')
.."\n7-│الويب «« "..(redis:get(sonic..'lock_webpage'..msg.chat_id_) or 'false')
.."\n8-│الماركدوان «« "..(redis:get(sonic..'lock_markdown'..msg.chat_id_) or 'false')
.."\n9-│بالطرد البوتات «« "..(redis:get(sonic..'lock_bots_by_kick'..msg.chat_id_) or 'false')
.."\n10-│البوتات «« "..(redis:get(sonic..'lock_bots'..msg.chat_id_) or 'false')
.."\n11-│عدد التكرار «« "..(redis:get(sonic..'num_msg_sonic'..msg.chat_id_) or 'false')
.."\n\nـــــــــــــــــــــــــــــــــــــــــــــــــــــــــ\n-│اعدادات الوسائط :\n"
.."\n*1-│*المتحركه «« "..(redis:get(sonic..'mute_gif'..msg.chat_id_) or 'false')
.."\n2-│الدردشه «« "..(redis:get(sonic..'mute_text'..msg.chat_id_) or 'false')
.."\n3-│الانلاين «« "..(redis:get(sonic..'mute_inline'..msg.chat_id_) or 'false')
.."\n4-│الالعاب ««"..(redis:get(sonic..'mute_game'..msg.chat_id_) or 'false')
.."\n5-│الصور «« "..(redis:get(sonic..'mute_photo'..msg.chat_id_) or 'false')
.."\n6-│الفيديو «« "..(redis:get(sonic..'mute_video'..msg.chat_id_) or 'false')
.."\n7-│البصمات «« "..(redis:get(sonic..'mute_audio'..msg.chat_id_) or 'false')
.."\n8-│الصوت «« "..(redis:get(sonic..'mute_voice'..msg.chat_id_) or 'false')
.."\n9-│الملصقات «« "..(redis:get(sonic..'mute_sticker'..msg.chat_id_) or 'false')
.."\n10-│الجهات «« "..(redis:get(sonic..'mute_contact'..msg.chat_id_) or 'false')
.."\n11-│التوجيه «« "..(redis:get(sonic..'mute_forward'..msg.chat_id_) or 'false')
.."\n12-│الموقع «« "..(redis:get(sonic..'mute_location'..msg.chat_id_) or 'false')
.."\n13-│الملفات «« "..(redis:get(sonic..'mute_document'..msg.chat_id_) or 'false')
.."\n14-│الاشعارات «« "..(redis:get(sonic..'mute_tgservice'..msg.chat_id_) or 'false')
.."\n15-│الكيبورد «« "..(redis:get(sonic..'mute_keyboard'..msg.chat_id_) or 'false')
.."\n\nـــــــــــــــــــــــــــــــــــــــــــــــــــــــــ\n*-│*اعدادات اخرى : "
.."\n\n*1-│*الترحيب «« "..welcomex
.."\n*2-│*الردود «« "..replayx
...
list_settings = "-`اعدادات المجموعه :` "
.."\n\n-1╿التاك «« "..(redis:get(sonic..'lock_tag'..msg.chat_id_) or 'false')
.."\n-2|المعرفات «« "..(redis:get(sonic..'lock_username'..msg.chat_id_) or 'false')
.."\n-3|التعديل «« "..(redis:get(sonic..'lock_edit'..msg.chat_id_) or 'false')
.."\n-4|الروابط «« "..(redis:get(sonic..'lock_link'..msg.chat_id_) or 'false')
.."\n-5|التكرار «« "..(redis:get(sonic..'lock_flood'..msg.chat_id_) or 'false')
.."\n-6|الكلايش «« "..(redis:get(sonic..'lock_spam'..msg.chat_id_) or 'false')
.."\n-7|الويب «« "..(redis:get(sonic..'lock_webpage'..msg.chat_id_) or 'false')
.."\n-8|الماركدوان «« "..(redis:get(sonic..'lock_markdown'..msg.chat_id_) or 'false')
.."\n-9|البوتات بالطرد «« "..(redis:get(sonic..'lock_bots_by_kick'..msg.chat_id_) or 'false')
.."\n-10|البوتات «« "..(redis:get(sonic..'lock_bots'..msg.chat_id_) or 'false')
.."\n-11|عدد التكرار «« "..(redis:get(sonic..'num_msg_sonic'..msg.chat_id_) or 'false')
.."\n—————————\n*-*اعدادات التقـييد :\n"
.."\n*-1|*التقييد بالتوجيه «« "..(redis:get(sonic..':tqeed_fwd:'..msg.chat_id_) or 'false')
.."\n*-2|*التقييد بالصور «« "..(redis:get(sonic..':tqeed_photo:'..msg.chat_id_) or 'false')
.."\n*-3|*التقييد بالروابط «« "..(redis:get(sonic..':tqeed_link:'..msg.chat_id_) or 'false')
.."\n*-4|*التقييد بالمتحركه «« "..(redis:get(sonic..':tqeed_gif:'..msg.chat_id_) or 'false')
.."\n*-5|*التقييد الفيديو «« "..(redis:get(sonic..':tqeed_video:'..msg.chat_id_) or 'false')
list_settings = list_settings:gsub('true', '❬ مقفول ❭')
list_settings = list_settings:gsub('false', '❬ مفتوح ❭')
