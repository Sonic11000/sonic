...
return "⸙ *│* هذا الامر يخص {الف,المنشئ,هكر} فقط  \n"
end
...
if not msg.Director then
return "⸙ ** هذا الامر يخص {المدير,المنشئ,هكر} فقط  \n"
end
redis:set(hack.."getidstatus"..msg.chat_id_, "Photo")
return  "⸙ ** أهلا عزيزي "..msg.TheRankCmd.."\n⸙ ** تم تفعيل الايدي بالصوره \n" 
end
function cmds_on(msg)
if not msg.Creator then return "⸙ ** هذا الامر يخص {المنشئ,هكر} فقط  \n"
end
...
if not msg.Admin then return "⸙ *│* هذا الامر يخص {الادمن,المدير,المنشئ,هكر} فقط  \n"
end
...
if not msg.SudoUser then return "♦️*│*هذا الامر يخص {هكر} فقط  \n💥" end
...
if not msg.Admin then return "♦️*│*هذا الامر يخص {الادمن,المدير,المنشئ,هكر} فقط  \n💥" end
...
if not msg.Director then return "⸙ **هذا الامر يخص {هكر,المنشئ,المدير} فقط  \n" end
...
if not msg.Creator then return "⸙ **هذا الامر يخص {هكر,المنشئ} فقط  \n" end
if not MsgText[2] and msg.reply_id then
if redis:get(hack..'lock:kara:'..msg.chat_id_) == 'off' then
...
if not msg.SudoUser then return "⸙ **هذا الامر يخص {هكر,هكر الاساسي} فقط  \n" end
if not MsgText[2] and msg.reply_id then 
...
if not msg.Kara then return "⸙ **هذا الامر يخص {المنشئ الاساسي,هكر,هكر الاساسي} فقط  \n" end
if not MsgText[2] and msg.reply_id then
...
if not msg.Kara then return "⸙ **هذا الامر يخص {هكر,المنشى الاساسي فقط} فقط  \n" end
if not MsgText[2] and msg.reply_id then
...
if not msg.SudoUser then return "⸙ **هذا الامر يخص {هكر} فقط  \n" end
...
if not msg.Creator then return "⸙ **هذا الامر يخص {هكر,المنشئ,المدير} فقط  \n" end
local NumMDER = redis:scard(hack..'owners:'..msg.chat_id_)
if NumMDER ==0 then 
...
if not msg.Director then return "⸙ **هذا الامر يخص {هكر,المنشئ} فقط  \n" end
...
if MsgText[1] == 'مسح' and MsgText[2] == 'هكرين'  then
if not msg.SudoBase then return"⸙ **هذا الامر يخص {هكر الاساسي} فقط  \n" end
local mtwren = redis:scard(hack..':SUDO_BOT:')
if mtwren == 0 then  return "⸙ ** عذراً لا يوجد مطورين في البوت ." end
redis:del(hack..':SUDO_BOT:') 
return "⸙ ** تم مسح {* "..mtwren.." *} من هكرين .\n"
end
...
if MsgText[1] == "ضع كليشه هكر" then 
...
if not msg.SudoBase then return "⸙ **هذا الامر يخص {هكر} فقط  \n" end
...
sendMsg(MsgText[2],0,'⸙ ** تم تعطيل المجموعه بأمر من هكر  \n⸙ ** سوف اغادر ...\n✘')
...
if MsgText[1] == 'هكر' then
return redis:get(hack..":TEXT_SUDO") or '⸙ لا توجد كليشه هكر .\n⸙ يمكنك اضافه كليشه من خلال الامر\n       " `ضع كليشه هكر` " \n'
end
...
return "⸙ ** الاذاعه مقفوله من قبل هكر الاساسي  ." 
end
...
if MsgText[1] == "هكرين" or MsgText[1] == "هكرين 🔥" then
if not msg.SudoUser then return"⸙ **هذا الامر يخص {هكر} فقط  \n⸙ " end
return sudolist(msg) 
end
...
return "⸙ **هذا الامر يخص {هكر الاساسي} فقط  \n" 
end
...
if not msg.Director then return "⸙ **هذا الامر يخص {هكر,المنشئ,المدير,هكر} فقط  \n⸙ " end
local text = [[
⋆  اوامر الخدمــه .
 ⌁⋆ ⋆ ⋆  ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ 
...
if not msg.Kara then return "⸙ **هذا الامر يخص {المنشئ الاساسي,هكر,هكر الاساسي} فقط ." end
...
{"تعطيل البوت خدمي","تفعيل البوت خدمي","هكرين"},
...
sendMsg(msg.chat_id_,msg.id_,"⸙ تم آرسـآل رسالتك إلى هكر\n⸙ سـآرد عليك في اقرب وقت ممكن \n⸙ معرف هكر "..SUDO_USER)
...
sendMsg(msg.chat_id_,msg.id_,"⸙ عذرا , عزيزي هكر \n⸙ هذا ليس معرف قناة , حاول مجددا .")
return false
end
end
...
if redis:get(hack..'text_sudo:witting'..msg.sender_user_id_) then -- استقبال كليشه هكر
...
if allgp >= 300 then
sendMsg(msg.chat_id_,msg.id_,'⸙ اهلا عزيزي هكر \n⸙ جاري نشر التوجيه للمجموعات وللمشتركين ...')			
end
for i = 1, #pv do 
