...
local points = redis:get(SONIC..':User_Points:'..msg.chat_id_..msg.sender_user_id_)
...
if redis:get(SONIC.."getidstatus"..msg.chat_id_) == "Photo" then
	if data.photos_[0] then 
		...
		sendPhoto(msg.chat_id_,msg.id_,data.photos_[0].sizes_[1].photo_.persistent_id_,'➥ 𝑁𝐴𝑀𝐸🇪🇬'..Namei..'    \n➥ 𝑈𝑆𝐸𝑅𝑁𝐴𝑀𝐸🇪🇬 '..UserNameID..' .\n➥  𝐺𝑀𝐴𝑆🇪🇬 '..msgs..' .\n➥ 𝑆𝑇𝐴𝑆𝑇🇪🇬'..msg.TheRank..' .\n➥  𝐼𝐷 🇾🇪'..msg.sender_user_id_..' .\n ➥ 𝐶𝐻 🇪🇬 @SouRce_SONIC .\n▁',dl_cb,nil)
		else
...
return '↯ **  عدد جهاتك المضافة‏‏ ⇜ ❪ '..(redis:get(SONIC..':adduser:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)..' ❫ \n.'
end
...
sendPhoto(msg.chat_id_,msg.id_,Photo_Weloame,[[↯ اهلا انا بوت]]..redis:get(SONIC..':NameBot:')..[[ .
...
return'↯ ** شـرط التفعيل البوت اذا كانت المجموعه‏‏ اكثر مـن *【'..redis:get(SONIC..':addnumberusers')..'】* عضـو .\n' 
end 
end
...
return redis:get(SONIC..":TEXT_SUDO") or '↯ لا توجد كليشه المطور .\n↯ يمكنك اضافه كليشه من خلال الامر\n       " `ضع كليشه المطور` " \n'
end
...
sendMsg(msg.chat_id_,msg.id_,'↯ ** عذراً اسم الملف غير مدعوم للنظام او لا يتوافق مع سورس ↯سونيك يرجاء جلب الملف الاصلي الذي قمت بسحبه وبدون تعديل ع الاسم\n')
end  
else
...
return [[
 ✧ 𝒔𝒐𝒖𝒓𝒄𝒆 𝒔𝒐𝒏𝒊𝒄 ✧
 ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ 
• 𝒅𝒆𝒗 >> [☆𝒂𝒍𝒂𝒎𝒚☆](t.me/Alamy3)
⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ 
• 𝒅𝒆𝒗 >> [☆𝒂𝒍𝒍𝒂𝒎☆](t.me/M_A_L_L_A_M)
⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ 
• • >> [☆ 𝒔𝒐𝒖𝒓𝒄𝒆_𝒔𝒐𝒏𝒊𝒄☆](t.me/SouRce_SONIC)
⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ 
• •>> [☆𝒕𝒐 𝒕𝒂𝒍𝒌 𝒖𝒔☆](t.me/FlassTas4_bot)
⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆
⇨⇨⇨⇨ 𝒔𝒐𝒖𝒓𝒄𝒆 𝒔𝒐𝒏𝒊𝒄 ⇦⇦⇦⇦
]]
end
...
𝙲𝙷 𖣲. :[source SONICer 𖡶 ](https://t.me/SouRce_hacker1)
]]
end
...
⌔︙كلب بلدي
 — — — — — — — — —
𝙳𝙴𝚅 𖡇 : [Alamy 𖡼](https://t.me/Alamy3)
𝙲𝙷 𖣲. :[source SONICer𖡶 ](https://t.me/SouRce_hacker1)
]]
end
...
if MsgText[1] == "احمد كامل" or MsgText[1]=="احمد كامل" then
return [[
...
return '↯ **عدد سحكاتك ⇜ ❪ '..(redis:get(SONIC..':edited:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)..' ❫ \n'
end
...
function WelcomeByAddUser(SONIC,Diamond)
...
------set cmd------
SONIC = msg.text 
if SONIC == 'رفع مشرف كامل' and msg.reply_to_message_id_ ~= 0 then

if not msg.Kara then return "↯ **هذا الامر يخص {المنشئ الاساسي,المطور,المطور الاساسي} فقط ." end

function setadmins(SONIC,diamond)
...
if SONIC == 'رفع مشرف' and msg.reply_to_message_id_ ~= 0 then
...
if SONIC == 'تنزيل مشرف' and msg.reply_to_message_id_ ~= 0 then

if not msg.Kara then return "↯ **هذا الامر يخص {المنشئ الاساسي,المطور,المطور الاساسي} فقط ." end

function remadmins(SONIC,diamond)
...
if SONIC:match('تغير امر (.*)') then
if not msg.Kara then return "↯ **هذا الامر يخص {المنشئ الاساسي,المطور,المطور الاساسي} فقط  \n" end
local cmd = SONIC:match('تغير امر (.*)') 
...
if SONIC and (SONIC:match('^delcmd (.*)') or SONIC:match('^مسح امر (.*)')) then
if not msg.Kara then return "↯ **هذا الامر يخص {المنشئ الاساسي,المطور,المطور الاساسي} فقط  \n" end
local cmd = SONIC:match('^delcmd (.*)') or SONIC:match('^مسح امر (.*)')
redis:hdel(SONIC..'CmD:'..msg.chat_id_,cmd)
redis:srem(SONIC..'CmDlist:'..msg.chat_id_,cmd)
sendMsg(msg.chat_id_,msg.id_,"↯ اهلا عزيزي\nالامر >"..cmd.."\n↯ تم مسحه من قائمه الاوامر\n")
end
if SONIC == 'مسح قائمه الاوامر' or SONIC == 'مسح قائمه الاوامر' then
if not msg.Kara then return "↯ **هذا الامر يخص {المنشئ الاساسي,المطور,المطور الاساسي} فقط  \n" end
...
if SONIC == 'welcome on' or SONIC == 'تفعيل الترحيب' then
if redis:get(SONIC..'welc'..msg.chat_id_) == 'on' then
...
text = [[↯ أهلاً انا بوت   []]..redis:get(SONIC..':NameBot:')..[[] 🗽
...
sendPhoto(msg.chat_id_,msg.id_,redis:get(SONIC..':WELCOME_BOT'),[[↯أهلاً انا بوت ]]..redis:get(SONIC..':NameBot:')..[[
↯ أختصاصي حماية‌‏ المجموعات
↯ مـن السبام والتوجيه‌‏ والتكرار والخ...
...
GetUserID(msg.sender_user_id_,function(arg,data)  
welcome = (redis:get(SONIC..'welcome:msg'..msg.chat_id_) or "↯ اهلاً عزيزي {الاسم}\n↯ معرفك » {المعرف}\n{القوانين}\n\nالرجاء الالتزام بالقوانين\nـــــــــــــــــــــــــــــــــــــــــــــــــــــــــ\n↯ اسم القروب » {المجموعه}")
if welcome then
rules = (redis:get(SONIC..'rulse:msg'..msg.chat_id_) or "↯ مرحباً عزيري القوانين كلاتي .\n↯ ممنوع نشر الروابط\n↯ ممنوع التكلم او نشر صور اباحيه\n↯ ممنوع  اعاده توجيه\n↯ ممنوع التكلم بالسياسة\n↯ الرجاء احترام المدراء والادمنيه .\n")
...
SendMention(msg.chat_id_,datau.id_,msg.id_,"↯ العضو » "..USERNAME.."\n↯ قمـت بتكرار اكثر مـن "..NUM_MSG_SONIC.." رسالة‌‏ , لذا تم تقييدك مـن المجموعه‌\n",12,USERCAR) 
return false
end)
end 
...
elseif Text== "علام" or Text== "مطور سورس" then return  sendMsg(msg.chat_id_,msg.id_,"[مطور سورس سونيك ↯](https://t.me/M_A_L_L_A_M)")
